...
Trade 09-24でプラトーを確認できたため、次はトレード回数増加を検証する。

### 目的

`00:00〜09:00` 除外を少し緩め、`08:00〜24:00` で取引した場合に、PFを維持しつつTradesが増えるか確認する。
...
```text
Tradesが増え、PF 1.5以上を維持できるか確認する。
PFが大きく落ちる場合、08時台は追加しない。
```
